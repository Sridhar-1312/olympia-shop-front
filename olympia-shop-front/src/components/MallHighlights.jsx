...
const MallHighlights = () => {
  ...
  const handlePrev = () => {
    setStartIndex((prev) =>
      prev === 0 ? allHighlights.length - cardsPerView : prev - cardsPerView
    );
  };

  const handleNext = () => {
    setStartIndex((prev) =>
      prev + cardsPerView >= allHighlights.length ? 0 : prev + cardsPerView
    );
  };

  const visibleCards = allHighlights.slice(
    startIndex,
    ...
    <section className="py-20 px-6 relative overflow-hidden">
      ...
      <div className="container mx-auto">
        ...
        <div className="relative">
          <button
            onClick={handlePrev}
            className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-slate-900/70 hover:bg-orange-500/80 text-white rounded-full p-3 shadow-lg transition-all duration-300 ease-in-out"
            aria-label="Previous"
          >
            <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M15 18l-6-6 6-6"/></svg>
          </button>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-8 transition-all duration-500 ease-in-out">
            {visibleCards.map((highlight, index) => (
              <div 
          ...
          <button
            onClick={handleNext}
            className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-slate-900/70 hover:bg-orange-500/80 text-white rounded-full p-3 shadow-lg transition-all duration-300 ease-in-out"
            aria-label="Next"
          >
